...
#[inline(always)]
pub(crate) const fn memset<const N: usize>(mut dest: [u8; N], offset: usize, val: u8, n: usize) -> [u8; N] {
    let mut i = 0;
    while i < n {
        dest[offset + i] = val;
        i += 1;
...
/// This function exists because subslices are not supported in `const fn`.
#[inline(always)]
pub(crate) const fn store_u128_be<const N: usize>(dest: [u8; N], offset: usize, n: u128) -> [u8; N] {
